...
function normalizeRequestRecord(record) {
  ...
  const exteriorColorRequested = textValue(fields["Exterior Color Requested"]);

  const r1Sent = booleanValue(fields["R1 Sent"]);
  const r1SentAt = isoValue(fields["RFQ Round 1 Sent At"]);
  const round1ResponseDeadlineAt = isoValue(
    fields["Round 1 Response Deadline At"],
  );
