...
import { createQuoteWindowExpiry } from "@/lib/utils/tokens";
...
export const DEALER_SELECTION_STATUS = {
  ...
  ROUND1_SEND_READY: "Round 1 Send Ready",
  ROUND1_SENT: "Round 1 Sent",
  ROUND2_SEND_READY: "Round 2 Send Ready",
...
function getWaitingOnBuyerStatus(currentRound) {
  return currentRound === "R2"
    ? REQUEST_STATUS.WAITING_ON_BUYER_R2
    : REQUEST_STATUS.WAITING_ON_BUYER_R1;
}

function buildRoundCloseFields({ currentRound, lifecycle, timestamp }) {
  const fields = {
    "Request Status": getWaitingOnBuyerStatus(currentRound),
    "Review Ready?": lifecycle?.flags?.shouldEnableBuyerReview === true,
  };

  if (currentRound === "R1") {
    fields["Round 1 Closed At"] = timestamp;
  }
...
function buildWorkflowGates({
  ...
  return {
    ...
    canEvaluateRoundLifecycle:
      !isClosed &&
      lifecycleFlags.isRoundStarted === true &&
      lifecycleFlags.isRoundClosedPersisted !== true,

    canCloseCurrentRound:
      !isClosed && lifecycleFlags.shouldCloseRoundNow === true,

    canMarkBuyerReviewReady:
      !isClosed &&
      lifecycleFlags.shouldEnableBuyerReview === true &&
      (lifecycleFlags.isRoundClosedPersisted === true ||
        lifecycleFlags.shouldCloseRoundNow === true),
...
export async function evaluateCurrentRoundLifecycle(
  requestInput,
  { now } = {},
) {
  ...
  return {
    ...
    gates: buildWorkflowGates({
      requestData: snapshot.requestData,
      dealerSelection: snapshot.dealerSelection,
      invitations: snapshot.invitations,
      currentRound: snapshot.currentRound,
      lifecycle,
    }),
...
export async function evaluateAndCloseCurrentRound(requestInput, { now } = {}) {
  const snapshot = await evaluateCurrentRoundLifecycle(requestInput, { now });

  if (!snapshot.gates.canEvaluateRoundLifecycle) {
    return {
      action: "evaluate-and-close-current-round",
      status: "not-eligible",
      snapshot,
  ...
  if (!lifecycle?.flags?.shouldCloseRoundNow) {
    return {
      action: "evaluate-and-close-current-round",
      status: "still-open",
      snapshot,
  ...
  const timestamp = lifecycle?.timestamps?.now || nowIso();
  const fields = buildRoundCloseFields({
    currentRound,
    lifecycle,
    timestamp,
  });

  await base.safeUpdate(REQUESTS_TABLE, snapshot.requestData.recordId, fields);

  const refreshedSnapshot = await getWorkflowSnapshot(
...
export async function markReadyForInvitations(requestInput) {
  ...
  if (!snapshot.gates.canMarkReadyForInvitations) {
    if (snapshot.requestData?.flags?.isClosed) {
      throw new Error(
        "Cannot mark Ready for Invitations because this Request is closed",
      );
    }

    if ((snapshot.invitations?.total ?? 0) > 0) {
      throw new Error(
        "Cannot mark Ready for Invitations because invitation records already exist",
      );
    }

    if ((snapshot.dealerSelection?.candidateCount ?? 0) < 1) {
      throw new Error(
        "Cannot mark Ready for Invitations without candidate dealers",
      );
    }

    throw new Error("Cannot mark Ready for Invitations");
  }
...
export async function markRound1SendReady(requestInput) {
  ...
  return updateRequestFields(snapshot.requestData.requestId, {
    "Dealer Selection Status": DEALER_SELECTION_STATUS.ROUND1_SEND_READY,
    "Round Number": "R1",
  });
}
...
export async function stampRound1Sent(requestInput) {
  ...
  const quoteExpiresAt = createQuoteWindowExpiry(
    timestamp,
    roundRules.roundWindowHours,
  );
  ...
  await batchUpdateInvitationFields(uniqueRecordIds(selectedRows), {
    ...
    "Dealer Quote Token Expires At": quoteExpiresAt,
  });

  return updateRequestFields(snapshot.requestData.requestId, {
    "Request Status": REQUEST_STATUS.BIDDING_LIVE_R1,
    "Dealer Selection Status": DEALER_SELECTION_STATUS.ROUND1_SENT,
    "Round Number": "R1",
...
export async function markInvitationSentAndMaybeFinalize(
  requestInput,
  { requestDealerRecordId, roundNumber = "R1" } = {},
) {
  const snapshot = await getWorkflowSnapshot(requestInput);

  if (!requestDealerRecordId) {
    throw new Error("requestDealerRecordId is required");
  }

  const normalizedRound = String(roundNumber || "R1")
    .trim()
    .toUpperCase();

  if (normalizedRound !== "R1") {
    throw new Error("Only Round 1 is supported by this helper right now");
  }

  const targetRow = (snapshot.invitations.rows || []).find(
    (row) => row.recordId === requestDealerRecordId,
  );

  if (!targetRow) {
    throw new Error("RequestDealer row not found for this Request");
  }

  if (targetRow.selectionStatus !== INVITATION_SELECTION_STATUS.SELECTED) {
    throw new Error("Only selected RequestDealer rows can be marked sent");
  }

  if (targetRow.roundNumber !== "R1") {
    throw new Error("Only Round 1 RequestDealer rows are supported here");
  }

  const alreadyRequestFinalized = Boolean(
    snapshot?.requestData?.workflow?.sendStatus?.r1SentAt,
  );

  if (
    targetRow.rfqSendStatus !== RFQ_SEND_STATUS.SENT ||
    !targetRow.rfqSentAt
  ) {
    const timestamp = nowIso();

    await batchUpdateInvitationFields([requestDealerRecordId], {
      "RFQ Send Status": RFQ_SEND_STATUS.SENT,
      "RFQ Sent At": timestamp,
      "Dealer Response Status": DEALER_RESPONSE_STATUS.AWAITING_RESPONSE,
    });
  ...
  const selectedRound1Rows = getSelectedInvitationRows(
    refreshedSnapshot.invitations.rows,
    "R1",
  );

  const allSent =
    selectedRound1Rows.length > 0 &&
    selectedRound1Rows.every(
      (row) =>
        row.selectionStatus === INVITATION_SELECTION_STATUS.SELECTED &&
        row.rfqSendStatus === RFQ_SEND_STATUS.SENT,
    );

  if (!alreadyRequestFinalized && allSent) {
    const updatedSnapshot = await stampRound1Sent(
      refreshedSnapshot.requestData.requestId,
    );
...
export async function markBuyerReviewReady(requestInput) {
  const snapshot = await evaluateCurrentRoundLifecycle(requestInput);
  ...
  return updateRequestFields(snapshot.requestData.requestId, fields);
}
