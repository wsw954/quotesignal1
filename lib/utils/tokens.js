...
export function addHoursToDate(dateInput, hours) {
  ...
  const safeHours =
    Number.isFinite(Number(hours)) && Number(hours) > 0 ? Number(hours) : 0;

  return new Date(base.getTime() + safeHours * MS_PER_HOUR);
}

export function createQuoteWindowExpiry(from, hours = 72) {
  return addHoursToDate(from, hours).toISOString();
}
