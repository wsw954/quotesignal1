...
function isSelectableDealerStatus(status) {
  const normalized = normalizeTitle(status);
  return normalized === "Active" || normalized === "New";
}

function resolveTemplateHint(status) {
  return normalizeTitle(status) === "New" ? "Intro" : "Standard";
}
...
function buildDealerEvaluation(dealer, requestData) {
  ...
  const dealerStatus = normalizeTitle(dealer.dealerStatus);
  const hasEligibleStatus = isSelectableDealerStatus(dealerStatus);

  if (hasEligibleStatus) {
    if (dealerStatus === "New") {
      fitReasons.push(
        "Dealer status is New (eligible and should receive Intro RFQ template)",
      );
    } else {
    ...
    exclusionReasons.push(
      dealerStatus
        ? `Dealer status is ${dealerStatus}`
        : "Dealer is not eligible by status",
    );
  }
  ...
  const isCandidate =
    hasEligibleStatus && geographyMatch && makeMatch && purchaseTypeMatch;

  let matchScore = 0;
  if (hasEligibleStatus) matchScore += 40;
  ...
  return {
    ...dealer,
    dealerStatus,
    isActive: dealerStatus === "Active",
    isNewDealer: dealerStatus === "New",
    geometryMatch: geographyMatch,
    makeMatch,
    purchaseTypeMatch,
    isCandidate,
    ...
    templateTypeHint: resolveTemplateHint(dealerStatus),
  };
}
